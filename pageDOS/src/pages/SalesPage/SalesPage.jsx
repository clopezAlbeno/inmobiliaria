...
import { SalesPropertyCard } from "../../components/SalesPropertyCard/SalesPropertyCard.jsx";
...
export const SalesPage = () => {
    ...
    const fetchProperties = async () => {
      try {
        ...
        const mappedData = response.data.map((prop) => ({
          ...
          currency: "MXN",
          transactionType: prop.tipo_operacion === "alquiler" ? "EN RENTA" : "EN VENTA",
          image: prop.imagenes?.find((img) => img.es_principal)?.url_imagen || prop.imagenes?.[0]?.url_imagen || "",
          bedrooms: prop.habitaciones,
          bathrooms: prop.banos,
          area: prop.metros,
          description: prop.descripcion,
          advisor: prop.usuario?.nombre || "No asignado",
          location: `${prop.ciudad || ""}, ${prop.departamento || ""}`,
          propertyType: prop.tipo_propiedad || "Casa",
          publishDate: prop.fecha_publicacion
        }));
  ...
  const salePropertiesData = useMemo(() => 
    propertiesData.filter(property => property.transactionType === "EN VENTA"),
    [propertiesData]
  );
  ...
  // Opciones únicas (solo de propiedades en venta)
  const uniqueLocations = [...new Set(salePropertiesData.map((prop) => prop.location))];
  const uniquePropertyTypes = [...new Set(salePropertiesData.map((prop) => prop.propertyType))];

  // Filtrar y ordenar
  const filteredProperties = useMemo(() => {
    let filtered = salePropertiesData.filter((property) => {
    ...
    return filtered;
  }, [filters, sortBy, salePropertiesData]);
      ...
      <div className="sales-page-content">
        ...
        <main className="properties-list">
          <div className="sort-bar">
            <span>{filteredProperties.length} propiedades en venta encontradas</span>
          ...
          {currentProperties.length > 0 ? (
            ...
            <div className="no-results">
              <h3>No se encontraron propiedades en venta</h3>
